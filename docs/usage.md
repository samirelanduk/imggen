# samirelanduk/nf-diffuser: Usage
...
```bash
nextflow run samirelanduk/nf-diffuser -profile docker -params-file params.yaml
```
...
```bash
nextflow pull samirelanduk/nf-diffuser
```
...
First, go to the [samirelanduk/nf-diffuser releases page](https://github.com/samirelanduk/nf-diffuser/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
